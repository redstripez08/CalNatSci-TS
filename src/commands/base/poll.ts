...
async function pollEmbed(message: Message, pollOptions: PollOptions) {
    ...
    if (!charCounter(text, 2048)) return message.channel.send("Poll is too large to send!\nTry shortening your options");
    ...
    const voterInfo = new Collection<string, {emoji: string}>();
    ...
    reactionCollector.on('collect', (reaction, user) => {
        if (usedEmojis.includes(reaction.emoji.name)) {
            ...
            if (reaction.emoji.name === forceEndPollEmoji) return;
            ...
            if (!votedEmoji) return message.channel.send("what");
            // Checks if vote has been changed
            if (votedEmoji !== reaction.emoji.name) {
                const lastVote = poll.reactions.cache.get(votedEmoji);
                if (!lastVote) return message.channel.send("We weren't able to process that!");
...
export default {
    ...
    async execute(message, args) {
        ...
        if (!argsArr[0]) return message.reply('Poll title is not given.');
        if (!charCounter(argsArr[0], 256)) return message.reply("Too large");
        ...
        if (options.length > 10) return message.reply(`Please provide 10 or less choices.`);    
        if (typeof parseInt(argsArr[2]) !== "number") return message.reply("timeout is not a number");
